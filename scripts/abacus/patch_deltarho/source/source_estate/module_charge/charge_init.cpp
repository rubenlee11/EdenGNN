                      ...
                      const void* wfcpw)
{
    ...
    if (PARAM.inp.init_chg == "file" || PARAM.inp.init_chg == "auto" || PARAM.inp.init_chg == "drho")
    {
        ...
        else
        {
            for (int is = 0; is < nspin; ++is)
            {
                    ...
                    ucell.nat))
                {
                    ...
                    init with delta rho.  
                    */                
                    if (PARAM.inp.init_chg == "drho"){
                        double** rho_atom = new double*[nspin];
                        for (int is_ = 0; is_ < nspin; is_++)
                        {
                            rho_atom[is_] = new double[this->rhopw->nrxx];
                        }
                        this->atomic_rho(nspin, ucell.omega, rho_atom, strucFac, ucell);

                        for (int ir = 0; ir < this->rhopw->nrxx; ir++)
                        {
                            rho[is][ir] += rho_atom[is][ir];
                        }
