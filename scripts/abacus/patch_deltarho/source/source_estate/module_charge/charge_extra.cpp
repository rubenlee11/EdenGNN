...
#include "source_io/module_parameter/parameter.h" // added by li xiwen
...
// added by li xiwen 2026-05-06
void Charge_Extra::write_delta_rho(    
    const UnitCell& ucell,
    const Charge* chr,
    const Parallel_Grid &para_grid)
{
    // output difference charge density (ispin = 0 only)
    if (PARAM.inp.out_chg[0] == 3){
        std::stringstream ss;
        ss << PARAM.globalv.global_out_dir << "chg";
        ss << "delta.cube";

        double* delta_rho = new double[chr->rhopw->nrxx];
        for (int ir = 0; ir < chr->rhopw->nrxx; ir++)
        {
            delta_rho[ir] = delta_rho1[0][ir] / ucell.omega;
        }

        ModuleIO::write_vdata_palgrid(
            para_grid,
            delta_rho,
            0, // number of fermi energy
            1, // nspin = 1
            this->istep,
            ss.str(),
            0.0, // fermi energy
