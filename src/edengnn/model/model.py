...
from edengnn.data.io.abacus.pseudo import BASIS, BASIS_SIZE, BASIS_START, L_MAX
...
class HermitianOperator(torch.nn.Module):
    def __init__(self, config):
        ...
        self.onsite = OperatorOnsiteLayer(
            irreps_node_features=config.atom.conv.irreps_node_features,
            n_channels=config.operator.onsite.n_channels,
            l_max=L_MAX,
        )
        self.offsite = OperatorOffsiteLayer(
            ...
            l_max=L_MAX,
            n_channels=config.operator.offsite.n_channels,
    ...
    def forward(self, data, batch=None):
        ...
        return {
            "operator": torch.concatenate(
                [data["operator_onsite"], data["operator_offsite"]]
            ),
        }
